...
@section('script')
    <script>
        ...
        btnGenerarData.addEventListener('click', () => {
            ...
            fetch(`/get-asistencias-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    if (data.code != 0) {
                        Swal.fire({
                        ...
                        data.asistencias.forEach(item => {
                            ...
                            row.innerHTML = `
                                ...
                                <td>${item.usuario}</td>
                                <td>${item.cedula}</td>
                                <td>${item.primer_login}</td>
                                <td>${item.ultimo_logout}</td>
                            `;
        ...
        btnGuardarData.addEventListener('click', () => {
            ...
            fetch(`/save-asistencias-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    Swal.fire({
                        title: "Listo",
        ...
        btnEliminarData.addEventListener('click', () => {
            ...
            fetch(`/delete-asistencias-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    Swal.fire({
                        title: "Listo",
        ...
        btnGuardarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/save-asistencias-lotobet?fecha=${date}`);
                    if (!response.ok) {
        ...
        btnEliminarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/delete-asistencias-lotobet?fecha=${date}`);
                    if (!response.ok) {
                        const text = await response.text().catch(() => null);
                        throw new Error(text || `Error HTTP ${response.status}`);
                    }
