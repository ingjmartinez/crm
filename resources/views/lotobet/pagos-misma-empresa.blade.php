                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Pagos Misma Empresa</h4>
...
@section('script')
    <script>
        ...
        btnGenerarData.addEventListener('click', () => {
            ...
            fetch(`/get-pagos-misma-empresa-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    if (data.code != 0) {
                        Swal.fire({
                        ...
                        data.pagos.forEach(item => {
                            ...
                            row.innerHTML = `
                                ...
                                <td>${item.monto}</td>
                                <td>${fecha}</td>
                                <td>${item.pagado_agencia_id}</td>
                                <td>${item.plataforma_pago}</td>
                            `;
        ...
        btnGuardarData.addEventListener('click', () => {
            ...
            fetch(`/save-pagos-misma-empresa-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    Swal.fire({
                        title: "Listo",
        ...
        btnEliminarData.addEventListener('click', () => {
            ...
            fetch(`/delete-pagos-misma-empresa-lotobet?fecha=${fecha}`)
                .then(response => response.json())
                .then(data => {
                    Swal.fire({
                        title: "Listo",
        ...
        btnGuardarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/save-pagos-misma-empresa-lotobet?fecha=${date}`);
                    if (!response.ok) {
        ...
        btnEliminarDataFecha.addEventListener('click', async () => {
            ...
            try {
                ...
                for (let i = 0; i < dates.length; i++) {
                    ...
                    const response = await fetch(`/delete-pagos-misma-empresa-lotobet?fecha=${date}`);
                    if (!response.ok) {
                        const text = await response.text().catch(() => null);
                        throw new Error(text || `Error HTTP ${response.status}`);
                    }
