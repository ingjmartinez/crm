                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Estado de Resultado</h4>
                            ...
                            <div class="card-body">
                                <form id="form-estado-resultado" class="row g-3 align-items-end">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="d-flex flex-wrap gap-3 pt-1" id="tipos">
                                            @foreach ($tipos as $tipo)
                                                <div class="form-check form-check-inline mb-0">
                                                    <input class="form-check-input check-tipo-contable" type="checkbox" value="{{ $tipo }}" id="tipo-{{ \Illuminate\Support\Str::slug($tipo) }}" checked>
                                                    <label class="form-check-label" for="tipo-{{ \Illuminate\Support\Str::slug($tipo) }}">{{ $tipo }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                        <div class="form-text">Activa uno, varios o todos segun el bloque que quieras analizar.</div>
                                    </div>
                                    <div class="col-md-2">
                    ...
                    <div class="col-md-3">
                        <div class="card border-success border-opacity-25 h-100">
                            <div class="card-body">
                                ...
                                <div class="fs-3 fw-semibold text-success" id="resumen-ingresos">0.00</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card border-warning border-opacity-25 h-100">
                            <div class="card-body">
                                <div class="text-muted small">Costos</div>
                                <div class="fs-3 fw-semibold text-warning" id="resumen-costos">0.00</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card border-danger border-opacity-25 h-100">
                            <div class="card-body">
                                ...
                                <div class="fs-3 fw-semibold text-danger" id="resumen-gastos">0.00</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card border-primary border-opacity-25 h-100">
                            <div class="card-body">
                                <div class="text-muted small">Resultado del periodo</div>
                                <div class="fs-3 fw-semibold text-primary" id="resumen-resultado">0.00</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div id="estado-resultado-loader" class="text-center py-4 d-none">
                                    ...
                                    <div class="mt-2 text-muted">Cargando estado de resultado...</div>
                                </div>
                                    ...
                                    <table class="table table-bordered table-striped align-middle mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Descripcion</th>
                                                <th style="width: 160px;" id="th-periodo">Periodo</th>
                                                <th style="width: 160px;">Acumulado</th>
                                            </tr>
                                        </thead>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function obtenerTiposSeleccionados() {
                return Array.from(document.querySelectorAll('.check-tipo-contable:checked'))
                    .map(function (check) {
                        return check.value;
                    })
                    .filter(Boolean);
            ...
            function renderGrupo(grupo) {
                ...
                filas.push(`
                    <tr class="table-light">
                        <td colspan="4" class="fw-bold">${grupo.titulo}</td>
                    </tr>
                `);
                ...
                (grupo.cuentas || []).forEach(function (cuenta) {
                    filas.push(`
                        <tr>
                            ...
                            <td class="text-end">${formatoMonto(cuenta.periodo)}</td>
            ...
            function obtenerTextoTipos() {
                ...
                return tiposSeleccionados.length ? tiposSeleccionados.join(', ') : 'Sin bloques';
            }
            ...
            async function obtenerMetaCuentas(tiposSeleccionados) {
                ...
                const response = await fetch("{{ route('contabilidad.reportes.estado-resultado.meta') }}?" + params.toString(), {
                    headers: {
            ...
            async function consultarBloqueRango(tipo, fechaDesde, fechaHasta, cuentaOffset, cuentaLimit) {
                ...
                const response = await fetch("{{ route('contabilidad.reportes.estado-resultado.data') }}?" + params.toString(), {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
                    }
            ...
            function generarPdfEstadoResultado() {
                ...
                const resumen = [
                    ['Ingresos', resumenIngresos.textContent || '0.00'],
                    ['Costos', resumenCostos.textContent || '0.00'],
                    ['Gastos', resumenGastos.textContent || '0.00'],
                    ['Resultado del periodo', resumenResultado.textContent || '0.00'],
                ];
                ...
                const headers = ['Cuenta', 'Descripcion', thPeriodo.textContent || 'Periodo', 'Acumulado'];
                const rows = [];
                ...
                doc.autoTable({
                    ...
                    columnStyles: {
                        0: { cellWidth: 90 },
                        1: { cellWidth: pageWidth - (margin * 2) - 270 },
                        2: { cellWidth: 90, halign: 'right' },
                        3: { cellWidth: 90, halign: 'right' }
                    }
                });

                doc.save('estado_resultado.pdf');
            }
            ...
            async function cargarEstadoResultado() {
                ...
                try {
                    ...
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({
                            title: 'Consultando estado de resultado',
                    ...
                    resumenIngresos.textContent = formatoMonto(resumenAcumulado.ingresos_periodo);
                    resumenCostos.textContent = formatoMonto(resumenAcumulado.costos_periodo);
                    resumenGastos.textContent = formatoMonto(resumenAcumulado.gastos_periodo);
                    resumenResultado.textContent = formatoMonto(resumenAcumulado.resultado_periodo);
                    ...
                    tbody.innerHTML = gruposAcumulados.map(renderGrupo).join('') + `
                        <tr class="table-primary fw-bold">
                            ...
                            <td class="text-end">${formatoMonto(resumenAcumulado.resultado_periodo)}</td>
                            <td class="text-end">${formatoMonto(resumenAcumulado.resultado_acumulado)}</td>
                        </tr>
                    `;
