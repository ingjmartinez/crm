                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0">Tecnologia - Solicitudes</h4>
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="{{ route('inicio.index') }}">Inicio</a></li>
                                    <li class="breadcrumb-item"><a href="{{ route('tecnologia.index') }}">Tecnologia</a></li>
                                    <li class="breadcrumb-item active">Solicitudes de Tecnologia</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="alert alert-warning alert-border-left mb-4" role="alert">
                                ...
                                El modulo esta creado, pero la tabla de base de datos aun no existe. Debes ejecutar la migracion pendiente para empezar a registrar solicitudes.
                            </div>
                        </div>
                    </div>
                @endif
                        ...
                        <div class="card">
                            <div class="card-header border-0">
                                <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
                                    <div>
                                        <h5 class="card-title mb-1">Bandeja de solicitudes</h5>
                                        <p class="text-muted mb-0">Registro centralizado para averias y requerimientos de desarrollo.</p>
                                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped align-middle mb-0">
                                        ...
                                        <tbody id="solicitudesTableBody">
                                            <tr>
                                                <td colspan="10" class="text-center text-muted py-4">Cargando solicitudes...</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <div>
                        <h5 class="modal-title mb-1" id="modalSolicitudTitulo">Nueva solicitud</h5>
                    ...
                    <form id="formSolicitudTecnologia">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="titulo" name="titulo" maxlength="160" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Tipo</label>
                            ...
                            <div class="col-md-12" id="progresoContainer" style="display: none;">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    <label class="form-label mb-0">Progreso del desarrollo</label>
...
@section('script')
    <script>
        const TEC_SOLICITUDES_BASE_URL = '{{ url('/tecnologia/solicitudes') }}';
        const TEC_SOLICITUDES_LIST_URL = '{{ route('tecnologia.solicitudes.list') }}';
        const TEC_SOLICITUDES_REQUEST_CLOSE_BASE_URL = '{{ url('/tecnologia/solicitudes') }}';
        const TEC_SOLICITUDES_FINALIZE_BASE_URL = '{{ url('/tecnologia/solicitudes') }}';
        ...
        function bindTecnologiaEvents() {
            ...
            document.getElementById('tipo').addEventListener('change', syncTipoUI);
        ...
        function cargarSolicitudes(openSolicitudId = null) {
            ...
            if (openSolicitudId) params.append('solicitud_id', String(openSolicitudId));
            ...
            fetch(`${TEC_SOLICITUDES_LIST_URL}?${params.toString()}`, {
                ...
                .then(response => {
                    ...
                    if (openSolicitudId) {
                        const solicitud = (response.data || []).find(item => item.id === openSolicitudId);
                        if (solicitud) {
        ...
        function renderSolicitudes(items) {
            ...
            if (!items.length) {
                tableBody.innerHTML = '<tr><td colspan="10" class="text-center text-muted py-4">No hay solicitudes para los filtros seleccionados.</td></tr>';
                return;
            }
            ...
            tableBody.innerHTML = items.map(item => {
                return `
                    <tr>
                        ...
                        <td><span class="badge bg-${escapeHtml(item.badge_tipo)}-subtle text-${escapeHtml(item.badge_tipo)}">${escapeHtml(formatLabel(item.tipo))}</span></td>
        ...
        function abrirModalEdicion(id) {
            fetch(`${TEC_SOLICITUDES_BASE_URL}/${id}`, {
                ...
                .then(response => {
                    ...
                    document.getElementById('tipo').value = solicitud.tipo || 'averia';
                    document.getElementById('prioridad').value = solicitud.prioridad || 'media';
                    document.getElementById('descripcion').value = solicitud.descripcion || '';
                    document.getElementById('asignado_id').value = solicitud.asignado_id || '';
                    document.getElementById('estado').value = solicitud.estado || 'pendiente';
                    document.getElementById('progreso').value = solicitud.progreso ?? 0;
                    updateProgressUI(solicitud.progreso ?? 0);
                    document.getElementById('detalle_solucion').value = solicitud.detalle_solucion || '';
                    document.getElementById('estadoContainer').style.display = '';
                    document.getElementById('detalleSolucionContainer').style.display = '';
                    document.getElementById('progresoContainer').style.display = solicitud.tipo === 'desarrollo' ? '' : 'none';
                    document.getElementById('modalSolicitudTitulo').textContent = `${solicitud.ticket_codigo} - ${solicitud.titulo}`;
                    document.getElementById('modalSolicitudMeta').textContent =
                        `${solicitud.cierre_solicitado_at ? `Cierre solicitado por ${solicitud.cierre_solicitado_por} el ${solicitud.cierre_solicitado_at}` : `Solicitante: ${solicitud.solicitante} | Asignado: ${solicitud.asignado} | Creado: ${solicitud.creado_en || ''}`}`;

                    toggleFormDisabled(!solicitud.can_edit);
                    document.getElementById('progreso').disabled = !solicitud.can_manage_progress;
                    document.getElementById('btnGuardarSolicitud').style.display = solicitud.can_edit ? '' : 'none';
                    document.getElementById('btnSolicitarCierre').style.display = solicitud.can_request_close ? '' : 'none';
                    document.getElementById('btnFinalizarTicket').style.display = solicitud.can_finalize ? '' : 'none';
                    syncTipoUI();
                    modalSolicitud.show();
                })
                .catch(showRequestError);
        ...
        function syncTipoUI() {
            const isDesarrollo = document.getElementById('tipo').value === 'desarrollo';
            const progresoContainer = document.getElementById('progresoContainer');
            progresoContainer.style.display = isDesarrollo ? '' : 'none';
        ...
        function guardarSolicitud() {
            ...
            if (isEdit) {
                ...
                payload.progreso = document.getElementById('tipo').value === 'desarrollo'
                    ? Number(document.getElementById('progreso').value || 0)
                    : 0;
            ...
            Swal.fire({
                title: isEdit ? 'Actualizando solicitud...' : 'Registrando solicitud...',
            ...
            fetch(url, {
                ...
                .then(handleJsonResponse)
                .then(response => {
                    Swal.fire('Proceso completado', response.message || 'Solicitud guardada correctamente.', 'success');
                    modalSolicitud.hide();
                    cargarSolicitudes();
                })
                .catch(showRequestError);
