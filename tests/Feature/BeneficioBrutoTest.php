...
class BeneficioBrutoTest extends TestCase
{
    ...
    public function test_management_report_counts_unique_terminals_that_sold_each_product(): void
    {
        ...
        $this->withoutMiddleware([Authenticate::class, ForcePasswordChange::class])
            ...
            ->assertSee('PREMIOS SACADOS %')
            ->assertSee("filaPorcentaje('PREMIOS SACADOS %', 'Premios sacados / ventas de loterías', premiosSacados, ventasLoterias)", false)
            ->assertSee('RESULTADO BRUTO %')
            ->assertSee("filaPorcentaje('RESULTADO BRUTO %', 'Balance neto / ventas totales', balanceGeneralNeto, ventasTotales)", false)
            ->assertDontSee('DEVOLUCIÓN')
            ->assertSee('(numeradorConsolidado / denominadorConsolidado) * 100', false)
            ->assertSee('data.cell.styles.fontSize = 11', false)
